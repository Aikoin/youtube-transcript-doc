...
```
按 `>>` 说话人标记切分（无标记则退回定长切分），保证每段是完整发言而非半句。脚本同时删掉纯语气词的独立段。

### 4. 清洗 + 翻译
按 `references/clean-translate.md`：删语气词、删口吃重复、修 ASR 错误（人名/术语/公司名）、
逐段翻译，并把 ASR 合并的「提问+回答」按语义切点拆成有序 `segs`（每个 `{speaker, zh, en_keep}`；字幕没带名字就 `speaker=""`）。
近百段时拆 2–3 批用并行子 agent 处理，共享一张术语修正表，各自输出 `{index: {segs:[...], key:bool}}` 的 JSON，合并成一个 content 文件。

**内容清洗规则**：
- **中文段落用中文全角标点**：中文子句里的 `,;:?!` 一律转全角。判定用「CJK 相邻」——只在前一个或后一个字符是汉字时才转，保住英文 token 内的半角（如 `productpass.com`、`SSO:`、代码）。两个英文词之间但明显作中文分隔的逗号（如「Ambrosino,OpenAI」）手工补成全角。
- **删广告/赞助口播**：插播赞助（productpass、WorkOS、Mercury 之类）整段删。判据：凡「访问 xx 网站了解更多 / 用码 xx 打折」即广告。结尾的自我推广也删，只留道别/致谢。
- **保留俚语/行话原词**：别硬翻，如 `dogfooding（内部试用）`——保原文加简短括注。
- **合并被 ASR 拆断的同一句**：同一说话人、语义连续的碎段合并成一个 turn（首段吸收后段，后段清空 `{'speaker':'','zh':'','en_keep':''}`）；真正的短插话/附和（别人的一句「对」「没错」）保留成独立 turn。
- **高光卡时间戳指向金句真实出现处**：`highlights[].sec` 落在这句话实际被说出的秒数。金句不属于某节就别塞进那节的「重点」卡，节 summary 只写该节自己的主旨。
...
`content.json` 的 schema（`build_doc.py` 头部有完整示例）：
...
- `paras{}`：正文，以段索引为键。每段用 `segs`（有序子发言列表，每个 `{speaker, zh, en_keep}`）承载多说话人，外加 `key` 布尔值。`speaker` 只在字幕自带名字或非常确定时才填，否则留空 `""`。`en_keep` 只在值得展示英文原文的重点 seg 上填，普通闲聊留 `""`。单人段也可用扁平 `{speaker,zh,en_keep,key}`（无 segs）向后兼容。
- `_no_ts_sections`：要**隐藏全部时间戳**的小节索引数组（如 Cold Open 蒙太奇，逐句时间戳纯是噪音）。

**版式规则 v3**（`build_doc.py` 已内置）：
- 标题朴素，不在标题里塞"中英对照"之类标签。
- **正文用普通段落，不用卡片**：每个 turn 是普通 `<p>`。卡片只留给节「重点」、顶部高光、头部信息框。
- **中文为主线，英文原文作引用块**：每个 turn 先出中文普通段落；时间戳链接（+ ⭐ 若含高光）内联在首个中文段落开头。该 turn 若有 `en_keep`，英文原文渲染成中文下方的灰字 `<blockquote>` 引用块。
- **换说话人不硬安名字**：拆成有序 segs，换到的子段——有确定名字标 `名字：`，没名字用中性 **🗣️** 前缀，不编造说话人。
- **不整段加粗**：加粗只留给说话人名字前缀、节「重点」标签。
- **用卡片颜色区分强调角色**：节「重点」卡用**绿色 light-green**（💡）；顶部高光卡用**黄色 light-yellow**（⭐）；头部信息框用**蓝色 light-blue**（🎙️）。
- **每节标题下加一张 💡 绿色「重点」卡片**（可点击时间戳 + 一句话概述）；`_no_ts_sections` 里的节不带时间戳。
...
**职责划分**：本 skill 负责先想清楚「画什么、什么风格、怎么画」，写成一份 brief；`beautiful-feishu-whiteboard` skill 负责按 brief 渲染、写入画板、回查线上真图。规划到位才能一次画对：不写 brief 直接甩要点，产出的是「PPT 罗列」而非「关系地图」，风格构图只能靠绘图 skill 猜。`beautiful-feishu-whiteboard` 是通用引擎，本 skill 只是它的调用方,不要改它的内部逻辑。
